...
export class CreditCardAgent implements Agent {
  ...
  private async createAccountViaGLAgent(
    ...
    try {
      ...
      // Typically credit card liabilities are in the 20000-29999 range
      ...
      // Start with 20000 if no existing credit card accounts, or increment from the highest existing code
      let accountCode = "20000";
...
Output (just the brief description):`;
  ...
  public async findOrCreateCreditCardAccountForTransactions(
    ...
    try {
      ...
      // AI-powered extraction of last four digits
      let lastFourDigits = "unknown";
      let extractionConfidence = "low";
      
      console.log(`[CreditCardAgent] [${timestamp}] Starting AI-powered last four digits extraction`);
      
      try {
        // First try AI extraction
        const aiResult = await extractLastFourDigitsWithAI(statementInfo);
        
        if (aiResult.lastFourDigits && aiResult.confidence !== 'low') {
          lastFourDigits = aiResult.lastFourDigits;
          extractionConfidence = aiResult.confidence;
          console.log(
            `[CreditCardAgent] [${timestamp}] AI extracted last four digits: ${lastFourDigits} (confidence: ${aiResult.confidence}, method: ${aiResult.extractionMethod})`
          );
        } else {
          // Fallback to regex patterns
          console.log(`[CreditCardAgent] [${timestamp}] AI extraction failed or low confidence, using fallback`);
          const fallbackResult = extractLastFourDigitsFallback(statementInfo);
          
          if (fallbackResult.lastFourDigits) {
            lastFourDigits = fallbackResult.lastFourDigits;
            extractionConfidence = fallbackResult.confidence;
            console.log(
              `[CreditCardAgent] [${timestamp}] Fallback extracted last four digits: ${lastFourDigits} (confidence: ${fallbackResult.confidence}, method: ${fallbackResult.extractionMethod})`
            );
          } else {
            console.log(`[CreditCardAgent] [${timestamp}] Both AI and fallback extraction failed`);
          }
        }
      } catch (error) {
        console.error(`[CreditCardAgent] [${timestamp}] Error in last four digits extraction:`, error);
        
        // Emergency fallback to simple extraction
        if (statementInfo.lastFourDigits && /^\d{4}$/.test(statementInfo.lastFourDigits)) {
          lastFourDigits = statementInfo.lastFourDigits;
          extractionConfidence = "medium";
          console.log(`[CreditCardAgent] [${timestamp}] Using provided lastFourDigits as emergency fallback: ${lastFourDigits}`);
        }
      }
...
- IMPORTANT: Only extract information that is explicitly present in the document. Do not fabricate or guess any values that are not clearly visible.`;
      ...
      try {
        ...
        if (extractedInfo.lastFourDigits) {
          ...
          if (!isValidLastFour) {
            ...
            // AI-powered extraction of last four digits from statement number
            try {
              const aiResult = await extractLastFourDigitsWithAI({ statementNumber: extractedInfo.statementNumber });
              
              if (aiResult.lastFourDigits && aiResult.confidence !== 'low') {
                extractedInfo.lastFourDigits = aiResult.lastFourDigits;
                console.log(
                  `[CreditCardAgent] AI extracted last four digits from statement number: ${aiResult.lastFourDigits} (confidence: ${aiResult.confidence})`
                );
              } else {
                // Fallback to simple extraction
                const fallbackResult = extractLastFourDigitsFallback({ statementNumber: extractedInfo.statementNumber });
                if (fallbackResult.lastFourDigits) {
                  extractedInfo.lastFourDigits = fallbackResult.lastFourDigits;
                  console.log(
                    `[CreditCardAgent] Fallback extracted last four digits: ${fallbackResult.lastFourDigits}`
                  );
                }
              }
            } catch (error) {
              console.error('[CreditCardAgent] Error in AI last four digits extraction:', error);
              // Keep the original simple extraction as final fallback
              const digits = extractedInfo.statementNumber.replace(/[^0-9]/g, "");
              if (digits.length >= 4) {
                const lastFour = digits.slice(-4);
                if (/^\d{4}$/.test(lastFour)) {
                  extractedInfo.lastFourDigits = lastFour;
                  console.log(`[CreditCardAgent] Emergency fallback extracted: ${lastFour}`);
                }
              }
            }
          } else {
  ...
  /**
   * Process statement with beginning balance integration
   * This method uses the extension to handle enhanced statement processing
   */
