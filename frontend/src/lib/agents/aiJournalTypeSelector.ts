...
export async function determineJournalType(
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    console.log('[aiJournalTypeSelector] No Anthropic API key found, using default journal types');
...
`;
    ...
    if (['CCP', 'CCY', 'CCR'].includes(journalType)) {
      console.log(`[aiJournalTypeSelector] Claude determined journal type: ${journalType}`);
      return journalType;
    } else {
      console.log(`[aiJournalTypeSelector] Invalid journal type from Claude: ${journalType}, using default`);
      if (isRefund) return DEFAULT_REFUND_TYPE;
      if (isPayment) return DEFAULT_PAYMENT_TYPE;
      return DEFAULT_PURCHASE_TYPE;
    }
  } catch (error) {
    console.error('[aiJournalTypeSelector] Error calling Claude:', error);
