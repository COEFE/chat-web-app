...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // --- DIAGNOSTIC PROMPT: Requesting simpler Excel JSON --- 
    let finalSystemPrompt = "You are a helpful assistant. If asked to create a spreadsheet from data (like a receipt), respond ONLY with the complete JSON to create the sheet and add the data rows. Use the 'action': 'createExcelFile' format with 'args.operations'. ONLY include 'createSheet' and 'addRow' operations. DO NOT include 'formatCell', 'formatRange', or 'setColumnWidth' operations. Example:\n\n{\n  \"action\": \"createExcelFile\",\n  \"args\": {\n    \"operations\": [\n      {\n        \"type\": \"createSheet\",\n        \"name\": \"Sheet1\"\n      },\n      {\n        \"type\": \"addRow\",\n        \"row\": [\"Header 1\", \"Header 2\"]\n      },\n      {\n        \"type\": \"addRow\",\n        \"row\": [\"Data A1\", \"Data B1\"]\n      }\n    ]\n  }\n}\n\nYour JSON response must be complete and not truncated. Do not add any introductory text, explanations, or concluding remarks around the JSON. If asked a general question or a question about the document content, answer normally.";
    // --- END DIAGNOSTIC PROMPT ---
    if (fileContent) {
